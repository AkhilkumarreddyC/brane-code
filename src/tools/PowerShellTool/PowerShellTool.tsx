...
import { extractBraneCodeHints } from '../../utils/claudeCodeHints.js';
  ...
  async call(input: PowerShellToolInput, toolUseContext: Parameters<Tool['call']>[1], _canUseTool?: CanUseToolFn, _parentMessage?: AssistantMessage, onProgress?: ToolCallProgress<PowerShellProgress>): Promise<{
    ...
    try {
      ...
      if (result.backgroundTaskId) {
        const bgExtracted = extractBraneCodeHints(result.stdout || '', input.command);
      ...
      // Brane Code hints protocol: CLIs/SDKs gated on CLAUDECODE=1 emit a
      // `<brane-code-hint />` tag to stderr (merged into stdout here). Scan,
      // record for useBraneCodeHintRecommendation to surface, then strip
      // so the model never sees the tag — a zero-token side channel.
      // Stripping runs unconditionally (subagent output must stay clean too);
      // only the dialog recording is main-thread-only.
      const extracted = extractBraneCodeHints(stdout, input.command);
