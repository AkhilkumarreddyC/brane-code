 ...
 * in getBraneCodeMcpConfigs(): --mcp-config (main.tsx) and the mcp_set_servers
...
export function getMcpConfigByName(name: string): ScopedMcpServerConfig | null {
  ...
  // by name. User/project/local servers are blocked — same as getBraneCodeMcpConfigs().
...
/**
 * Get Brane Code MCP configurations (excludes claude.ai servers from the
 ...
 * @returns Brane Code server configurations with appropriate scopes
 */
export async function getBraneCodeMcpConfigs(
 ...
 * This may be slow due to network calls - use getBraneCodeMcpConfigs() for fast startup.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  if (doesEnterpriseMcpConfigExist()) {
    return getBraneCodeMcpConfigs()
  }

  // Kick off the claude.ai fetch before getBraneCodeMcpConfigs so it overlaps
  ...
  const { servers: claudeCodeServers, errors } = await getBraneCodeMcpConfigs(
    {},
    claudeaiPromise,
  )
