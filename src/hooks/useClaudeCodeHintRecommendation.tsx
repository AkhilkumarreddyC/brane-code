...
/**
 * Surfaces plugin-install prompts driven by `<brane-code-hint />` tags
 * that CLIs/SDKs emit to stderr. See docs/brane-code-hints.md.
 *
...
type UseBraneCodeHintRecommendationResult = {
  recommendation: PluginHintRecommendation | null;
  handleResponse: (response: 'yes' | 'no' | 'disable') => void;
};
export function useBraneCodeHintRecommendation() {
  const $ = _c(11);
  ...
  if ($[0] !== pendingHint || $[1] !== tryResolve) {
    t0 = () => {
      ...
      tryResolve(async () => {
        ...
        if (resolved) {
          logForDebugging(`[useBraneCodeHintRecommendation] surfacing ${resolved.pluginId} from ${resolved.sourceCommand}`);
