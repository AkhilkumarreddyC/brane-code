...
import { aggregateBraneCodeStatsForRange, type BraneCodeStats, type DailyModelTokens, type StatsDateRange } from '../utils/stats.js';
...
function createAllTimeStatsPromise(): Promise<StatsResult> {
  return aggregateBraneCodeStatsForRange('all').then((data): StatsResult => {
...
export function Stats(t0) {
  ...
  if ($[1] === Symbol.for("react.memo_cache_sentinel")) {
    t2 = <Box marginTop={1}><Spinner /><Text> Loading your Brane Code stats…</Text></Box>;
    $[1] = t2;
  } else {
    t2 = $[1];
  }
...
function StatsContent(t0) {
  ...
  if ($[1] !== dateRange || $[2] !== statsCache) {
    t2 = () => {
      ...
      aggregateBraneCodeStatsForRange(dateRange).then(data => {
        if (!cancelled) {
          setStatsCache(prev => ({
            ...prev,
            [dateRange]: data
  ...
  if (allTimeResult.type === "empty") {
    ...
    if ($[15] === Symbol.for("react.memo_cache_sentinel")) {
      t7 = <Box marginTop={1}><Text color="warning">No stats available yet. Start using Brane Code!</Text></Box>;
      $[15] = t7;
    } else {
      t7 = $[15];
    }
...
function OverviewTab({
  ...
  stats: BraneCodeStats;
  allTimeStats: BraneCodeStats;
...
function generateFunFactoid(stats: BraneCodeStats, totalTokens: number): string {
...
async function handleScreenshot(stats: BraneCodeStats, activeTab: 'Overview' | 'Models', setStatus: (status: string | null) => void): Promise<void> {
  setStatus('copying…');
...
function renderStatsToAnsi(stats: BraneCodeStats, activeTab: 'Overview' | 'Models'): string {
  const lines: string[] = [];
...
function renderOverviewToAnsi(stats: BraneCodeStats): string[] {
  const lines: string[] = [];
  const theme = getTheme(resolveThemeSetting(getGlobalConfig().theme));
...
function renderModelsToAnsi(stats: BraneCodeStats): string[] {
  const lines: string[] = [];
  const modelEntries = Object.entries(stats.modelUsage).sort(([, a], [, b]) => b.inputTokens + b.outputTokens - (a.inputTokens + a.outputTokens));
  if (modelEntries.length === 0) {
