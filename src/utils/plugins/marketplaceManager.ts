/**
 * Marketplace manager for Brane Code plugins
 *
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Brane Code bug, shouldn't hit the error file.
    return {}
  }
}
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Brane Code version. Run 'claude marketplace remove ${name}' and ` +
  ...
  try {
    ...
    if (source.source === 'github' || source.source === 'git') {
      ...
      try {
        ...
        const reason =
          name === 'brane-code-plugins'
            ? `We've deprecated "brane-code-plugins" in favor of "claude-plugins-official".`
