...
/**
 * Convert a PersistedStatsCache to BraneCodeStats by computing derived fields.
 */
...
/**
 * Aggregates stats from all Brane Code sessions across all projects.
 * Uses a disk cache to avoid reprocessing historical data.
 */
export async function aggregateBraneCodeStats(): Promise<BraneCodeStats> {
 ...
 */
export async function aggregateBraneCodeStatsForRange(
  range: StatsDateRange,
): Promise<BraneCodeStats> {
  if (range === 'all') {
    return aggregateBraneCodeStats()
  }
  ...
  return processedStatsToBraneCodeStats(stats)
}

/**
 * Convert ProcessedStats to BraneCodeStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToBraneCodeStats(
...
function getEmptyStats(): BraneCodeStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
