/**
 * Brane Code hints protocol.
 *
 * CLIs and SDKs running under Brane Code can emit a self-closing
 * `<brane-code-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/brane-code-hints.md for the vendor-facing spec.
 */

import { logForDebugging } from './debug.js'
import { createSignal } from './signal.js'

export type BraneCodeHintType = 'plugin'

export type BraneCodeHint = {
  /** Spec version declared by the emitter. Unknown versions are dropped. */
  v: number
  /** Hint discriminator. v1 defines only `plugin`. */
  type: BraneCodeHintType
  /**
 ...
 */
const HINT_TAG_RE = /^[ \t]*<brane-code-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
 ...
 */
export function extractBraneCodeHints(
  output: string,
  command: string,
): { hints: BraneCodeHint[]; stripped: string } {
  // Fast path: no tag open sequence → no work, no allocation.
  if (!output.includes('<brane-code-hint')) {
    return { hints: [], stripped: output }
  }

  const sourceCommand = firstCommandToken(command)
  const hints: BraneCodeHint[] = []
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    hints.push({ v, type: type as BraneCodeHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: BraneCodeHint | null = null
...
export function setPendingHint(hint: BraneCodeHint): void {
...
export function getPendingHintSnapshot(): BraneCodeHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetBraneCodeHintStore(): void {
  pendingHint = null
